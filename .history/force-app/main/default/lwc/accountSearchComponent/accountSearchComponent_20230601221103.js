...
export default class AccountSearchComponent extends LightningElement {
    ...
    handleSearch() {
        ...
        getAccountRecords({state : this.state,city : this.city,industry : this.industry,rating : this.rating,name : this.name,phone : this.phoneNumber,offset :this.rowOffSet})
        .then(result => {
            //this.accountRecords = result;
            let updatedRecords = [...this.accountRecords, ...result];
            this.accountRecords = updatedRecords;
        })
        .catch(error => {
            this.accountRecords = undefined;
            if(error) {
    ...
    loadMoreDataHandler(event) {
        const currentRecord = this.accountRecords;
        const { target } = event;
